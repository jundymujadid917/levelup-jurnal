...
// ─── AVATAR FRAMES (unlock by level) ─────────────────────────────────────────

const AVATAR_FRAMES = [
  {
    id:"frame_basic", name:"Basic", unlockLevel:1, cost:0,
    style:{border:"3px solid #334155", boxShadow:"none"},
    label:"⬜ Basic",
  },
  {
    id:"frame_hunter", name:"Hunter", unlockLevel:3, cost:0,
    style:{border:"3px solid #3b82f6", boxShadow:"0 0 12px #3b82f688"},
    label:"🔵 Hunter",
  },
  {
    id:"frame_warrior", name:"Warrior", unlockLevel:5, cost:50,
    style:{border:"3px solid #ef4444", boxShadow:"0 0 14px #ef444488"},
    label:"🔴 Warrior",
  },
  {
    id:"frame_mage", name:"Mage", unlockLevel:8, cost:80,
    style:{border:"3px solid #a855f7", boxShadow:"0 0 16px #a855f788"},
    label:"🟣 Mage",
  },
  {
    id:"frame_gold", name:"Gold", unlockLevel:10, cost:120,
    style:{border:"3px solid #f59e0b", boxShadow:"0 0 18px #f59e0b99"},
    label:"🟡 Gold",
  },
  {
    id:"frame_shadow", name:"Shadow", unlockLevel:15, cost:200,
    style:{border:"3px solid #1e293b", boxShadow:"0 0 20px #000, inset 0 0 10px #0005"},
    label:"⚫ Shadow",
  },
  {
    id:"frame_sovereign", name:"Sovereign", unlockLevel:20, cost:500,
    style:{border:"3px solid #fbbf24", boxShadow:"0 0 24px #fbbf2499, 0 0 48px #fbbf2433"},
    label:"✨ Sovereign",
  },
];

// ─── AVATAR BADGES (overlay icon on frame) ───────────────────────────────────
...
const ASSESSMENT_STEPS = [
  {
    id:"current_state",phase:"KONDISI",icon:"🔍",
    label:"Kondisi fisik & mentalmu hari ini?",
    type:"single",
    options:[
      {value:"Sangat segar, siap tempur penuh",     emoji:"💪"},
      {value:"Oke, ada tenaga tapi tidak maksimal",  emoji:"🙂"},
      {value:"Capek tapi masih bisa bergerak",      emoji:"😮‍💨"},
      {value:"Lelah banget, butuh recovery",         emoji:"😴"},
    ],
  },
  {
    id:"habits_today",phase:"HARI INI",icon:"✅",
    label:"Kebiasaan baik yang sudah kamu lakukan hari ini?",
    type:"multi",
    options:[
      {value:"Olahraga / gerak fisik",  emoji:"🏃"},
      {value:"Sholat tepat waktu",      emoji:"🤲"},
      {value:"Tilawah Al-Qur'an",       emoji:"📖"},
      {value:"Belajar / membaca buku",  emoji:"📚"},
      {value:"Makan & minum sehat",     emoji:"🥗"},
      {value:"Tidur cukup semalam",     emoji:"😴"},
      {value:"Belum ada",               emoji:"😶"},
    ],
  },
  {
    id:"habits_build",phase:"TARGET",icon:"🎯",
    label:"Kebiasaan baik yang ingin kamu bangun?",
    type:"multi",
    options:[
      {value:"Olahraga rutin setiap hari",      emoji:"💪"},
      {value:"Konsisten sholat 5 waktu",        emoji:"🕌"},
      {value:"Tilawah minimal 1 halaman/hari",  emoji:"📖"},
      {value:"Baca buku 20 menit/hari",         emoji:"📚"},
      {value:"Tidur sebelum jam 22.00",         emoji:"🌙"},
      {value:"Kurangi screen time",             emoji:"📵"},
      {value:"Makan lebih sehat & teratur",     emoji:"🥗"},
    ],
  },
  {
    id:"habits_drop",phase:"TARGET",icon:"🚫",
    label:"Kebiasaan buruk yang ingin kamu buang?",
    type:"multi",
    options:[
      {value:"Begadang / tidur larut",        emoji:"🌃"},
      {value:"Scroll medsos berlebihan",       emoji:"📱"},
      {value:"Malas gerak / rebahan terus",   emoji:"🛋️"},
      {value:"Skip sholat / ibadah",          emoji:"😔"},
      {value:"Makan tidak teratur",           emoji:"🍟"},
      {value:"Prokrastinasi / menunda",       emoji:"⏰"},
      {value:"Emosi tidak terkontrol",        emoji:"😤"},
    ],
  },
  {
    id:"energy_level",phase:"ENERGI",icon:"⚡",
    label:"Level energimu saat ini?",
    type:"single",
    options:[
      {value:"10 — Full power!",              emoji:"🔋"},
      {value:"7 — Masih semangat",            emoji:"✅"},
      {value:"4 — Setengah tank",             emoji:"🟡"},
      {value:"1 — Hampir habis",              emoji:"🪫"},
    ],
  },
  {
    id:"focus_area",phase:"FOKUS",icon:"🌟",
    label:"Area yang paling ingin kamu kembangkan?",
    type:"single",
    options:[
      {value:"STR",label:"💪 Fisik & Kekuatan",     emoji:"💪"},
      {value:"AGI",label:"🏃 Kardio & Kelincahan",  emoji:"🏃"},
      {value:"VIT",label:"🛌 Kesehatan & Pemulihan",emoji:"🛌"},
      {value:"INT",label:"📖 Ilmu & Belajar",       emoji:"📖"},
      {value:"WIS",label:"🤲 Spiritual & Karakter", emoji:"🤲"},
    ],
  },
];

const INITIAL_STATE = {
  playerName:"",level:1,xp:0,coins:0,
  stats:{STR:10,AGI:10,VIT:10,INT:10,WIS:10},
  unlockedSkills:[],
  avatarImage:null,          // base64 gambar yang diupload
  equippedFrame:"frame_basic",
  equippedBadge:"none",
  ownedFrames:["frame_basic","frame_hunter"],
  ownedBadges:["none","sword"],
  quests:[],completedHistory:[],
  streak:0,lastQuestDate:null,
  setupDone:false,
  assessment:null,assessmentDate:null,
};
  ...
  return(
    ...
    >
      ...
      <div style={{
        ...
        overflow:"hidden",
        ...frame.style,
        background:avatarImage?"#000":"#1a1a2e",
      ...
      {showEdit&&(
        <div style={{
          ...
          background:"#533483",border:"2px solid #0a0a0f",
          borderRadius:"50%",
          width:size*0.3,height:size*0.3,
          display:"flex",alignItems:"center",justifyContent:"center",
          fontSize:size*0.14,zIndex:3,
        }}>✏️</div>
      )}
    </div>
  );
...
function AvatarScreen({state,onUpdate,onClose}){
  ...
  function buyFrame(frame){
    if(coins<frame.cost||ownedFrames.includes(frame.id))return;
    setCoins(c=>c-frame.cost);
    setOwnedFrames(f=>[...f,frame.id]);
    setPreviewFrame(frame.id);
  }
  function buyBadge(badge){
    if(coins<badge.cost||ownedBadges.includes(badge.id))return;
    setCoins(c=>c-badge.cost);
    setOwnedBadges(b=>[...b,badge.id]);
    setPreviewBadge(badge.id);
  }

  function handleSave(){
    onUpdate({
      avatarImage:previewImg,
      equippedFrame:previewFrame,
      equippedBadge:previewBadge,
      ownedFrames,ownedBadges,
      coins,
    });
    onClose();
  }
  ...
  return(
    <div style={{...css.root,position:"fixed",inset:0,zIndex:300,overflowY:"auto"}}>
      ...
      <div style={{display:"flex",borderBottom:"1px solid #1a1a2e",position:"sticky",top:49,background:"#0a0a0f",zIndex:9}}>
        {tabs.map(t=>(
          <button key={t.id} style={{flex:1,background:"none",border:"none",color:tab===t.id?"#4fc3f7":"#64748b",fontSize:12,padding:"11px 4px",cursor:"pointer",borderBottom:`2px solid ${tab===t.id?"#4fc3f7":"transparent"}`,letterSpacing:0.5}}
      ...
      <div style={{padding:20,paddingBottom:100}}>
        ...
        {tab==="photo"&&(
          <div>
            <div style={{fontSize:12,color:"#64748b",letterSpacing:1,marginBottom:16}}>Upload foto atau gambar untuk jadi avatarmu</div>

            {/* Upload button */}
            <div
              onClick={()=>fileRef.current.click()}
              style={{border:"2px dashed #334155",borderRadius:12,padding:"32px 20px",textAlign:"center",cursor:"pointer",marginBottom:16,background:"#0e1117",transition:"border-color 0.2s"}}
            >
              ...
              <div style={{fontSize:11,color:"#475569"}}>JPG, PNG, GIF — max 5MB</div>
            </div>
            <input ref={fileRef} type="file" accept="image/*" style={{display:"none"}} onChange={handleFileChange}/>

            {previewImg&&(
              <div style={{display:"flex",gap:10}}>
                <div style={{flex:1,background:"#0e1117",border:"1px solid #1e293b",borderRadius:10,overflow:"hidden",aspectRatio:"1"}}>
                  <img src={previewImg} alt="preview" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                </div>
                <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",gap:8}}>
                  <div style={{fontSize:12,color:"#94a3b8"}}>Foto berhasil diupload!</div>
                  <button style={{...css.btnSecondary,marginTop:0,fontSize:11,padding:"8px 12px"}} onClick={()=>fileRef.current.click()}>
                    🔄 Ganti
                  </button>
                  <button style={{background:"transparent",border:"1px solid #ef4444",color:"#ef4444",padding:"8px 12px",borderRadius:8,cursor:"pointer",fontSize:11,fontFamily:"monospace"}}
        ...
        {tab==="frame"&&(
          <div>
            <div style={{fontSize:12,color:"#64748b",letterSpacing:1,marginBottom:16}}>Frame unlock by level atau beli dengan koin</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {AVATAR_FRAMES.map(frame=>{
                const owned   = ownedFrames.includes(frame.id);
                const active  = previewFrame===frame.id;
                const canSee  = state.level>=frame.unlockLevel;
                return(
                  <div key={frame.id}
                    onClick={()=>owned&&setPreviewFrame(frame.id)}
                    style={{
                      background:active?"#1a2a4a":"#0e1117",
                      border:`1px solid ${active?"#4fc3f7":"#1e293b"}`,
                      borderRadius:10,padding:"14px 12px",
                      cursor:owned?"pointer":"default",
                      opacity:canSee?1:0.35,
                      textAlign:"center",transition:"all 0.15s",
                    }}
                  >
                    {/* Mini avatar preview with this frame */}
                    <div style={{display:"flex",justifyContent:"center",marginBottom:8}}>
                      <div style={{width:44,height:44,borderRadius:10,overflow:"hidden",...frame.style,background:"#1a1a2e",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>
                        {previewImg?<img src={previewImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"👤"}
                      </div>
                    </div>
                    <div style={{fontSize:12,color:active?"#e2e8f0":"#94a3b8",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,marginBottom:4}}>
                      {frame.label}
                    </div>
                    {active&&<div style={{fontSize:9,color:"#4fc3f7",letterSpacing:1}}>✓ AKTIF</div>}
                    {!owned&&canSee&&(
                      <button
                        onClick={e=>{e.stopPropagation();buyFrame(frame);}}
                        style={{marginTop:6,background:"transparent",border:`1px solid ${coins>=frame.cost?"#fbbf24":"#475569"}`,color:coins>=frame.cost?"#fbbf24":"#475569",padding:"4px 10px",borderRadius:5,cursor:coins>=frame.cost?"pointer":"not-allowed",fontSize:10,fontFamily:"monospace",width:"100%"}}
                      >🪙 {frame.cost}</button>
                    )}
                    {!owned&&!canSee&&<div style={{fontSize:9,color:"#475569",marginTop:4}}>LV.{frame.unlockLevel}</div>}
        ...
        {tab==="badge"&&(
          <div>
            <div style={{fontSize:12,color:"#64748b",letterSpacing:1,marginBottom:16}}>Badge muncul di sudut avatar</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {AVATAR_BADGES.map(badge=>{
                const owned  = ownedBadges.includes(badge.id);
                const active = previewBadge===badge.id;
                const canSee = state.level>=badge.unlockLevel;
                return(
                  <div key={badge.id}
                    onClick={()=>owned&&setPreviewBadge(badge.id)}
                    style={{
                      background:active?"#1a2a4a":"#0e1117",
                      border:`1px solid ${active?"#4fc3f7":"#1e293b"}`,
                      borderRadius:10,padding:"14px 12px",
                  ...
                  >
                    <div style={{fontSize:30,marginBottom:6}}>{badge.emoji||"∅"}</div>
                    <div style={{fontSize:12,color:active?"#e2e8f0":"#94a3b8",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,marginBottom:4}}>
                      {badge.label}
                    </div>
                    {active&&<div style={{fontSize:9,color:"#4fc3f7",letterSpacing:1}}>✓ AKTIF</div>}
                    {!owned&&canSee&&(
                      <button
                        onClick={e=>{e.stopPropagation();buyBadge(badge);}}
                        style={{marginTop:6,background:"transparent",border:`1px solid ${coins>=badge.cost?"#fbbf24":"#475569"}`,color:coins>=badge.cost?"#fbbf24":"#475569",padding:"4px 10px",borderRadius:5,cursor:coins>=badge.cost?"pointer":"not-allowed",fontSize:10,fontFamily:"monospace",width:"100%"}}
                      >{badge.cost===0?"GRATIS":`🪙 ${badge.cost}`}</button>
                    )}
                    {!owned&&!canSee&&<div style={{fontSize:9,color:"#475569",marginTop:4}}>LV.{badge.unlockLevel}</div>}
                    {owned&&!active&&<div style={{fontSize:9,color:"#334155",marginTop:4,letterSpacing:1}}>TAP UNTUK PAKAI</div>}
                  </div>
                );
              })}
...
async function generateQuestsFromAI(player){
  ...
  const prompt = `You are the System AI in a Solo Leveling-style reality.
Hunter: ${player.playerName||"Hunter"} | Level ${player.level} (Rank ${rank})
Stats: ${statSummary}
Skills: ${player.unlockedSkills.join(", ")||"none"}
...
Assign 5 daily quests. Weight toward focus area, counter bad habits.
...
function AssessmentScreen({existing,onSave,onSkip}){
  ...
  function toggleMulti(v){
    setAnswers(a=>{const cur=Array.isArray(a[q.id])?a[q.id]:[];return{...a,[q.id]:cur.includes(v)?cur.filter(x=>x!==v):[...cur,v]};});
  }
  ...
  return(
    <div style={css.root}>
      ...
      <div style={{padding:"20px 20px 10px",borderBottom:"1px solid #1a1a2e"}}>
        <div style={{fontSize:10,color:"#64748b",letterSpacing:3,marginBottom:6,fontFamily:"monospace"}}>SYSTEM — HUNTER ASSESSMENT</div>
        <div style={{height:3,background:"#1a1a2e",borderRadius:3}}>
          <div style={{height:"100%",width:`${prog}%`,background:"linear-gradient(90deg,#533483,#4fc3f7)",borderRadius:3,transition:"width 0.4s ease"}}/>
        </div>
      ...
      <div style={{padding:"24px 20px",flex:1,overflowY:"auto"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:q.type==="multi"?0:16}}>
          {q.options.map(opt=>{
            ...
            return(
              <button key={optVal}
                style={{background:isSelected?"#1a2a4a":"#0e1117",border:`1px solid ${isSelected?"#4fc3f7":"#1e293b"}`,borderLeft:`3px solid ${isSelected?"#4fc3f7":"#1e293b"}`,color:isSelected?"#e2e8f0":"#94a3b8",padding:"13px 16px",borderRadius:8,cursor:"pointer",fontSize:13,textAlign:"left",fontFamily:"'Inter',sans-serif",transition:"all 0.15s",display:"flex",alignItems:"center",gap:10}}
      ...
      <div style={{padding:"0 20px 32px",display:"flex",gap:10}}>
        {step>0
          ?<button style={{...css.btnSecondary,flex:1}} onClick={()=>setStep(s=>s-1)}>← KEMBALI</button>
          :<button style={{...css.btnSecondary,flex:1}} onClick={onSkip}>LEWATI</button>
        }
        <button style={{...css.btnPrimary,flex:2,opacity:canNext?1:0.4,marginTop:0}} disabled={!canNext}
...
export default function App(){
  const [state,setState]           = useState(()=>{try{const s=localStorage.getItem("levelup_v5");return s?JSON.parse(s):INITIAL_STATE;}catch{return INITIAL_STATE;}});
  ...
  useEffect(()=>{try{localStorage.setItem("levelup_v5",JSON.stringify(state));}catch{}},[state]);
  ...
  const handleSaveAssessment = useCallback(async(answers)=>{
    ...
    setShowAssess(false);setLoading(true);
    showNotif("info","⚙️ Generating quest dari assessment kamu...");
    try{
  ...
  const handleGenerateQuests = useCallback(async()=>{
    ...
    setLoading(true);showNotif("info","⚙️ Analyzing hunter profile...");
    try{
  ...
  const completeQuest = useCallback((questId)=>{
    setState(prev=>{
      ...
      const coins=quest.coinReward||10;
      ...
      AVATAR_FRAMES.forEach(f=>{if(f.cost===0&&newLvl>=f.unlockLevel&&!newFrames.includes(f.id))newFrames.push(f.id);});
      const newBadges=[...(prev.ownedBadges||["none"])];
      AVATAR_BADGES.forEach(b=>{if(b.cost===0&&newLvl>=b.unlockLevel&&!newBadges.includes(b.id))newBadges.push(b.id);});

      if(newLvl>prev.level)setTimeout(()=>{setLvlUp(true);setTimeout(()=>setLvlUp(false),2600);},200);
      setTimeout(()=>showNotif("xp",`+${gained} XP  ·  🪙+${coins}  ·  +${quest.statReward} ${quest.stat}${streakBonus>1?"  🔥":""}`,3000),100);

      return{...prev,quests:prev.quests.map(q=>q.id===questId?{...q,completed:true}:q),xp:newXP,level:newLvl,stats:newStats,unlockedSkills:newSkills,coins:(prev.coins||0)+coins,ownedFrames:newFrames,ownedBadges:newBadges,completedHistory:[...(prev.completedHistory||[]),{...quest,completedAt:Date.now(),date:today()}]};
    });
  },[showNotif]);
    ...
    <div style={css.root}><style>{FONTS}</style>
      <div style={css.setupWrap}>
        ...
        <button style={{...css.btnPrimary,opacity:setupName.trim()?1:0.4}} disabled={!setupName.trim()}
          onClick={()=>setState(s=>({...s,playerName:setupName.trim(),setupDone:true}))}>▶ BEGIN AWAKENING</button>
      </div>
    </div>
  );
  ...
  return(
    <div style={{...css.root,filter:glitch?"brightness(1.3) contrast(1.1)":"none",transition:"filter 0.1s"}}>
      ...
      {levelUpAnim&&(
        <div style={css.overlay}>
          <div style={css.levelBox}>
            ...
            <div style={{fontSize:12,color:"#64748b",marginTop:4}}>Frame & badge baru mungkin terbuka!</div>
            <button style={{...css.btnPrimary,marginTop:16,fontSize:11,padding:"8px 20px",width:"auto"}}
              onClick={()=>{setLvlUp(false);setShowAvatar(true);}}>✏️ Customize Avatar</button>
          </div>
        </div>
      )}
      {skillAnim&&(
      ...
      <div style={css.header}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <AvatarDisplay
            ...
            size={52} showEdit onClick={()=>setShowAvatar(true)}
          />
          <div>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <div style={{textAlign:"right"}}>
            ...
            <div style={{fontSize:10,color:"#64748b"}}>{state.xp.toLocaleString()} XP</div>
          </div>
      ...
      {!hasAssessToday&&(
        <div onClick={()=>setShowAssess(true)} style={{margin:"10px 20px 0",padding:"10px 14px",background:"#160f2e",border:"1px solid #533483",borderRadius:8,cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:18}}>📋</span>
          <div>
            <div style={{fontSize:11,color:"#a78bfa",letterSpacing:1,fontFamily:"monospace"}}>ASSESSMENT BELUM DIISI</div>
            <div style={{fontSize:10,color:"#64748b",marginTop:2}}>Isi untuk quest yang lebih personal</div>
          </div>
      ...
      <div style={css.nav}>
        {["quests","stats","skills"].map(t=>(
          <button key={t} style={{...css.navBtn,...(activeTab===t?css.navActive:{})}} onClick={()=>setActiveTab(t)}>
            {t==="quests"?"⚔️ QUESTS":t==="stats"?"📊 STATS":"✨ SKILLS"}
          </button>
        ))}
      </div>
        ...
        <div style={css.tab}>
          ...
          {todayQuests.length===0?(
            <div style={css.empty}>
              ...
              <div style={{color:"#475569",fontSize:11,marginTop:4}}>{state.assessment?"Tekan GET QUESTS":"Isi assessment dulu"}</div>
              {!state.assessment&&<button style={{...css.btnPrimary,marginTop:16,fontSize:11}} onClick={()=>setShowAssess(true)}>📋 ISI ASSESSMENT</button>}
            </div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {todayQuests.map(q=><QuestCard key={q.id} quest={q} onComplete={completeQuest}/>)}
            </div>
          )}
        </div>
      )}
      ...
      {activeTab==="stats"&&(
        <div style={css.tab}>
          <div style={{display:"flex",justifyContent:"center",marginBottom:20,cursor:"pointer"}} onClick={()=>setShowAvatar(true)}>
            <div style={{background:"#0a0a14",border:"1px solid #1e293b",borderRadius:16,padding:"24px 48px",textAlign:"center"}}>
          ...
          <div style={{marginTop:12,padding:"12px 16px",background:"#0f1620",borderRadius:8,border:"1px solid #1e293b"}}>
            ...
            <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
              {[{val:state.completedHistory?.length||0,label:"QUESTS",color:"#4fc3f7"},{val:state.unlockedSkills.length,label:"SKILLS",color:"#a78bfa"},{val:state.coins||0,label:"KOIN",color:"#fbbf24"},{val:state.level,label:"LEVEL",color:"#ef4444"}].map(x=>(
                <div key={x.label}>
                  <div style={{fontSize:18,fontWeight:900,color:x.color,fontFamily:"monospace"}}>{x.val}</div>
...
function QuestCard({quest,onComplete}){
  ...
  return(
    <div style={{background:quest.completed?"#0a120a":"#0e1117",border:`1px solid ${quest.completed?"#1a3a1a":"#1e293b"}`,borderLeft:`3px solid ${quest.completed?"#22c55e":(m.color||"#4fc3f7")}`,borderRadius:10,padding:"14px 16px",opacity:quest.completed?0.55:1,transition:"all 0.2s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
        <div style={{flex:1}}>
          <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:4}}>
            <span style={{fontSize:9,color:diffColor[quest.difficulty]||"#64748b",border:`1px solid ${diffColor[quest.difficulty]||"#64748b"}`,padding:"1px 6px",borderRadius:3,letterSpacing:1}}>{quest.difficulty}</span>
            <span style={{fontSize:9,color:m.color||"#4fc3f7",letterSpacing:1}}>{m.icon} {quest.stat}</span>
          </div>
          <div style={{fontSize:14,fontWeight:700,color:quest.completed?"#475569":"#e2e8f0",fontFamily:"'Rajdhani',sans-serif",lineHeight:1.3,letterSpacing:0.5}}>{quest.title}</div>
          <div style={{fontSize:11,color:"#64748b",marginTop:4}}>{quest.action}</div>
        </div>
        <div style={{textAlign:"right",marginLeft:12,minWidth:52}}>
          <div style={{fontSize:13,fontWeight:700,color:"#4fc3f7",fontFamily:"monospace"}}>+{quest.xpReward}</div>
          <div style={{fontSize:10,color:"#fbbf24",fontFamily:"monospace"}}>🪙{quest.coinReward||10}</div>
        </div>
      </div>
      {!quest.completed
